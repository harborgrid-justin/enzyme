...
export function Settings(): React.ReactElement {
  ...
  const auditEvents = useCmsStore((s) => s.auditEvents);
  const canManage = hasPermission(CMS_PERMISSIONS.MANAGE_SETTINGS);
  ...
  return (
    <section className="page-stack">
      <PageHeader
        ...
        description="Toggle CMS capabilities and inspect audit events. Feature-flag changes apply instantly via enzyme's `FeatureFlagProvider.setFlag`."
      />
        ...
        <Panel title="Feature flags" icon={<Flag size={18} />}>
          {!canManage && (
            <p className="muted small">
              Read-only — managing flags requires the admin role.
            </p>
          )}
