...
export function Topbar(): React.ReactElement {
  const activeWorkspace = useInventoryStore((s) => s.activeWorkspace);
  ...
  return (
    <header className="topbar">
      <div>
        ...
        <h1>Warehouse operations console</h1>
      </div>
      <div className="topbar-actions">
        <div className={`search-box ${advancedSearch ? 'advanced' : ''}`} aria-hidden="true">
          <Search size={16} />
          <span>{advancedSearch ? 'Search SKUs, suppliers, locations…' : 'Search'}</span>
        </div>
        <ThemeToggle />
        <UserMenu />
      </div>
