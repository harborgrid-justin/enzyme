...
): z.ZodDefault<z.ZodNumber> | z.ZodNumber {
...
export const hydrationConfigSchema = z.object({
  ...
  interaction: z
    ...
    .default({
      triggerEvents: ['click', 'focus', 'touchstart', 'mouseenter'],
      useCapture: true,
      hydrateOnHover: true,
      hoverDelay: 100,
    }),
 ...
 */
export const morphTransitionSchema = z
  .object({
...
export const layoutsConfigSchema = z.object({
  ...
  morphing: z
    ...
    .default({
      enabled: true,
      transition: { duration: 300, easing: 'ease-out', useGPU: true, respectReducedMotion: true },
      animate: true,
      useFLIP: true,
    }),
 ...
 */
const cspDirectiveSchema = z.union([z.string(), z.array(z.string())]);
 ...
 */
export function generateSchemaDocumentation(schema: z.ZodTypeAny, name: string): string {
  const lines: string[] = [
    `## ${name}`,
    '',
